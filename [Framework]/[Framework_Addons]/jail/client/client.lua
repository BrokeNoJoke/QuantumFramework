...
RegisterNUICallback("jailuser", function(data)
    SetNuiFocus(false, false)
    TriggerServerEvent("NAT2K15:JAILUSERCHECK", data.id, data.length)
    DeleteObject(BoxModel)
    ClearPedTasksImmediately(GetPlayerPed(-1))
    ClearPedTasks(GetPlayerPed(-1))
end)
...
-- OPEN Ui -- 
RegisterNetEvent("NAT2K15:OPENDAJAILUI")
AddEventHandler("NAT2K15:OPENDAJAILUI", function(playerstuff)
...
-- JAIL THE USER -- 
RegisterNetEvent("NAT2K15:GETJAILED")
AddEventHandler("NAT2K15:GETJAILED", function(id, length)
    local ped = GetPlayerPed(-1)
    local x,y,z = table.unpack(GetEntityCoords(ped))
    x, y, z = tonumber(x), tonumber(y), tonumber(z)
    Citizen.CreateThread(function()
...
RegisterNetEvent("NAT2K15:UNJAILAALL")
AddEventHandler("NAT2K15:UNJAILAALL", function(id, name) 
...
RegisterNetEvent('NAT2K15:JAILCHECK')
AddEventHandler('NAT2K15:JAILCHECK', function(playerid) 
    ...
    if(currentJailed == true) then
        ...
        currentJailed = false
        SetEntityInvincible(ped, false)
        TriggerServerEvent('NAT2K15:JAILRETURNCHECK', myserverid, playerid, true)
    else 
        TriggerServerEvent('NAT2K15:JAILRETURNCHECK', myserverid, playerid, false)
    end
end)
