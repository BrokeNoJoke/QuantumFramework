RegisterNetEvent('NAT2K15:GETJAILPLAYERS')
AddEventHandler('NAT2K15:GETJAILPLAYERS', function(src) 
    local allplayers = {}
    local num = 0
    ...
    Wait(10)
    TriggerClientEvent('NAT2K15:OPENDAJAILUI', src, allplayers)
end)


RegisterNetEvent("NAT2K15:JAILUSERCHECK")
AddEventHandler("NAT2K15:JAILUSERCHECK", function(id, length)
    ...
    if(playerstuff[id].dept == "Civilian") then
        for _, player in ipairs(GetPlayers()) do
            ...
            if(player == id) then
                ...
                TriggerClientEvent("NAT2K15:GETJAILED", player, id, length)
    ...
    if(player == nil or player[src].level ~= "admin_level") then
        ...
        if(playertocheck == nil or tonumber(playertocheck) == nil) then
            ...
            if(GetPlayerName(src) == nil) then
                ...
                TriggerClientEvent('NAT2K15:JAILCHECK', playertocheck, src)
            end
        end
    end
end)

RegisterNetEvent('NAT2K15:JAILRETURNCHECK')
AddEventHandler('NAT2K15:JAILRETURNCHECK', function(id, org, dead) 
    if(dead == true) then
...
RegisterCommand("unjailall", function(source, args, message) 
    ...
    if(playerstuff == nil or playerstuff[src].level ~= "admin_level") then
        ...
        for _, player in ipairs(GetPlayers()) do
            ...
            TriggerClientEvent('NAT2K15:UNJAILAALL', player, src, GetPlayerName(src))
        end
