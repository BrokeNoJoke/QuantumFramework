                ...
                if(clientstuff == nil) then 
                    ...
                    if(clientstuff[playerid].dept ~= "Civilian") then
                        ...
                        if IsControlJustReleased(1, Config.tabletkey) then
                            TriggerServerEvent('NAT2K15:GETSICKPEOPLE', playerid)
                        end
                    end
                end
            end
...
RegisterNUICallback("hospuser", function(data)
    SetNuiFocus(false, false)
    TriggerServerEvent("NAT2K15:HOSPITALUSERCHECK", data.id, data.length)
    DeleteObject(BoxModel)
    ClearPedTasksImmediately(GetPlayerPed(-1))
    ClearPedTasks(GetPlayerPed(-1))
end)
...
-- OPEN Ui -- 
RegisterNetEvent("NAT2K15:OPENDAHOSTLUI")
AddEventHandler("NAT2K15:OPENDAHOSTLUI", function(playerstuff)
...
RegisterNetEvent("NAT2K15:GETHOSPITAL")
AddEventHandler("NAT2K15:GETHOSPITAL", function(id, length)
    Citizen.CreateThread(function()
        curhospital = true
        local ped = GetPlayerPed(-1)
        Citizen.Wait(100)
